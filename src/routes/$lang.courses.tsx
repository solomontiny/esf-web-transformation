...
import { useCmsData, type CmsCourses } from "@/lib/cms";
...
function CoursesPage() {
  ...
  const fallback: CmsCourses = {
    eyebrow: t.courses.eyebrow,
    title: t.courses.title,
    lede: t.courses.lede,
    languages: t.courses.languages.map((c) => ({
      name: c.name,
      tagline: c.tagline,
      body: c.body,
  ...
  const cms = useCmsData("courses", fallback);
    ...
    <>
      <Section>
        ...
        <div className="mt-12 space-y-12">
          {cms.languages.map((course, index) => {
            const courseKey = courseKeys[index] ?? "english";

            return (
              <article key={course.name || index} className="grid gap-8 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:items-center">
                <img
